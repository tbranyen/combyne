  ...
  Compiler.prototype.process = function(nodes, keyVal) {
    ...
    nodes.map(function(node, index) {
      switch (node.type) {
        ...
        default: {
          // Offset the `\n` character at the beginning of a loop.
          if (keyVal && index === 0 && node.value === "\n") {
            commands.push("(data['" + keyVal[0] + "'] != 0 ? '\\n' : '')");
          }
          // Offset the `\n` character at the end of a loop, otherwise add
          // normally.
          else if (!keyVal || index !== length || node.value !== "\n") {
