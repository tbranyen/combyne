define(function(require, exports, module) {
  ...
  // FIXME Remove dependency on keyVal and instead inside compileLoop create a
  // new data object and scope everything inside of that.  That will keep this
  // function simple and less smart.

  function normalizeIdentifier(identifier, keyVal) {
    // i is always a key within a loop.
    if (identifier === "i" || (keyVal && identifier === keyVal[0])) {
      return keyVal[0];
    }

    // Dot is always value within a loop.
    if (identifier === "." || (keyVal && identifier === keyVal[1])) {
      return keyVal[1];
    }
  ...
  Compiler.prototype.process = function(body, keyVal) {
    var commands = [];
    ...
    body.forEach(function(entry) {
      switch (entry.type) {
        case "Property": {
          commands.push(this.compileProperty(entry, keyVal));
          break;
        }
  ...
  Compiler.prototype.compileProperty = function(entry, keyVal) {
    var identifier = normalizeIdentifier(entry.value, keyVal);
  ...
  Compiler.prototype.compileLoop = function(entry) {
    var keyVal = [
      ...
      // Value.
      (entry.conditions[2] ? entry.conditions[2].value : "data")
    ];

    var loop = [
      "map(", normalizeIdentifier(entry.conditions[0].value, keyVal), ",",
        "function(", keyVal.join(","), ") {",
