  ...
  Tree.prototype.make = function(root, END) {
    ...
    while (this.stack.length) {
      ...
      switch (node.name) {
        ...
        case "START_EXPR":
          if (node.previous && node.previous.name === "WHITESPACE") {
            prev = root.nodes[root.nodes.length - 1];

            if (prev.value.indexOf("\n") > -1) {
              prev.value = prev.value.slice(0, prev.value.indexOf("\n") + 1);
            }
          }
        ...
        case "WHITESPACE":
          ...
          if (node.previous.name === "END_EXPR") {
            ...
            if (capture.indexOf("\n") > -1) {
              ...
              // Wipe out whitespace between expressions.
              if (this.stack[0] && this.stack[0].name === "START_EXPR") {
                capture = "";
              }
            }
          }
