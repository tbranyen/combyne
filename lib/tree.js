define(function(require, exports, module) {
  ...
  Tree.prototype.constructProperty = function(rootEntry) {
    ...
    while (this.stack.length) {
      ...
      switch (entry.name) {
        ...
        case "MAGIC": {
          propertyDescriptor.type = "MagicProperty";
          break;
        }
  ...
  Tree.prototype.constructPartial = function(root) {
    ...
    var parseArguments = false;

    LOOP: while(this.stack.length) {
      var entry = this.stack.shift();
      ...
      switch (entry.name) {
        case "OTHER": {
          if (root.value === undefined) {
            ...
            root.arguments.push(entry.capture[0].trim());
          }
        }
  ...
  Tree.prototype.make = function(root, END) {
    ...
    while (this.stack.length) {
      ...
      switch (entry.name) {
        case "START_PROP": {
          if (result = this.constructProperty(root)) {
